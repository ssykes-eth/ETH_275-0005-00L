...
them into something the retriever can search. The pipeline has four steps:
...
class IngestionCore:
    ...
    def ingest_document(self, document: Document) -> list[Chunk]:
        ...
        # 2. BUILD — wrap each passage in a Chunk, carrying the document's metadata.
        chunks = [
            Chunk(
        ...
        # 4. STORE — ensure the collection (sized from the embedding dim), then persist.
